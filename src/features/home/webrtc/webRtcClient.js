export const myRTC = function() {
  ...
  myRTC.prototype.shareFile = function(fileList) {
    let socketId, that = this;
    for (socketId in that.dataChannels) that.sendFile(fileList, socketId);
  };
  //向某一单个用户发送文件
  myRTC.prototype.sendFile = function(fileList, socketId) {
  ...
  myRTC.prototype.sendFileAccept = function(sendId) {
    ...
    channel || that.emit('receive_file_error', new Error('Channel has been destoried'), sendId, fileInfo.socketId);
    var packet = { type: '__file', signal: 'accept', sendId: sendId };
    channel.send(JSON.stringify(packet));
  };
  //发送拒绝接受文件信令
